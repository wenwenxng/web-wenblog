...
var art = function () {
    ...
    var getArtDetail = function (callback) {
        var params = WWX.getParamByUrl()
        if (!params.art_id) return
        $.ajax({
            url: '/api/article/queryArticleDetail.php',
            type: 'get',
            dataType: 'json',
            data: {art_id: params.art_id},
    ...
    var getComments = function (callback) {
        ...
        $.ajax({
            ...
            type: 'get',
            dataType: 'json',
            data: {art_id: params.art_id, page: 1, pageSize: 5},
            success: function (data) {
    ...
    getArtDetail(function (artDetail) {
        getComments(function (comments) {
            window.comments = comments;
            console.log(artDetail);
            console.log(comments);
            var html = template('artTemplate', {art: artDetail, count:comments.count})
            $('#art_box').html(html)
            var commentHtml = template('firstComentTemplate', {comments: comments.data})
            $('#comment').html(commentHtml)
            $('title').html(artDetail.art_title)
        })
    })
}
...
var initAddFirstComment = function(){
    ...
    $('.wwx_article')
        ...
        var params = WWX.getParamByUrl()
        if (!params.art_id) return
        addFirstComment({art_id:params.art_id,content},function(data){
